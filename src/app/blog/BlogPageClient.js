...
function BlogContent({ initialPosts = INITIAL_POSTS, initialTotal = staticBlogPosts.length }) {
  ...
  useEffect(() => {
    if (!hasRenderedInitialData.current && currentPage === 1 && !searchTerm && selectedCategory === 'all' && !selectedTag) {
    ...
    setLoading(true);
    const timer = window.setTimeout(() => fetchPosts(controller.signal), delay);
    return () => {
