    ...
    if (gameState === 'loading') {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                <div className="animate-pulse text-2xl text-[#e8c547] font-bold">{tr ? 'Test Hazırlanıyor...' : 'Generating Quiz...'}</div>
            </div>
        );
    }
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fadeIn">
                <div className="bg-[#111] border border-[#e8c547]/30 rounded-2xl p-8 max-w-md w-full text-center relative overflow-hidden">
                    ...
                    <h2 className="text-3xl font-bold text-white mb-2">{tr ? 'Test Tamamlandı!' : 'Quiz Complete!'}</h2>
                    <p className="text-gray-400 mb-8">{tr ? <>Puanınız: <span className="text-[#e8c547] font-bold text-xl">{score}</span></> : <>You scored <span className="text-[#e8c547] font-bold text-xl">{score}</span> points</>}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <button onClick={generateQuiz} className="px-6 py-3 rounded-xl bg-[#e8c547] hover:bg-[#e8c547]/80 text-black font-bold transition-colors flex items-center justify-center gap-2">
                            <FaRedo /> {tr ? 'Tekrar Oyna' : 'Play Again'}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl p-6 md:p-8 max-w-2xl w-full relative shadow-2xl">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div className="text-sm text-gray-500 font-bold uppercase tracking-wider">
                        {tr ? 'Soru' : 'Question'} {currentQuestionIndex + 1}/{questions.length}
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="text-[#e8c547] font-bold">{tr ? 'Puan' : 'Score'}: {score}</div>
                ...
                <div className="mb-8 text-center">
                    <div className="mb-2 text-gray-400 text-sm">
                        {currentQ.type === 'term_to_meaning' ? (tr ? 'Anlamı nedir:' : 'What is the meaning of:') : (tr ? 'Bu anlama gelen kelime hangisi:' : 'Which word means:')}
                    </div>
