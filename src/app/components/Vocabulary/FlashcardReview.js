            ...
            <div className="bg-[#111] border border-white/10 rounded-2xl p-6 md:p-8 max-w-2xl w-full relative shadow-2xl">
                ...
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-[#e8c547]">
                        <FaLayerGroup /> {tr ? 'Aralıklı Tekrar' : 'Spaced Review'}
                    </div>
                    <div className="flex items-center gap-4">
                        <span className="text-sm text-gray-500">{Math.min(reviewed, total)}/{total || 0}</span>
                        <button onClick={onClose} aria-label={tr ? 'Tekrarı kapat' : 'Close review'} className="text-gray-500 hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e8c547]/60 rounded">
                            <FaTimes size={20} />
                        </button>
                    </div>
                </div>
                ...
                {finished ? (
                    <div className="text-center py-10">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-2">
                            {total === 0 ? (tr ? 'Şu anda tekrar edilecek kart yok' : 'Nothing to review right now') : (tr ? 'Tekrar tamamlandı!' : 'Review complete!')}
                        </h2>
                        <p className="text-gray-400 mb-8">
                            {total === 0
                                ? (tr ? 'Bazı kelimeleri “öğreniliyor” olarak işaretleyin veya kartların zamanı geldiğinde yeniden dönün.' : 'Mark some words as "learning" or come back when cards are due.')
                                : (tr ? `${reviewed} kartı tekrar ettiniz. Yeni tekrar tarihleri bu cihaza kaydedildi.` : `You reviewed ${reviewed} card${reviewed === 1 ? '' : 's'}. New due dates are saved on this device.`)}
                        </p>
                    ...
                    <>
                        ...
                        >
                            ...
                            {!flipped ? (
                                <p className="mt-6 text-xs text-gray-500 flex items-center gap-2"><FaSync /> {tr ? 'Göstermek için dokunun veya boşluk tuşuna basın' : 'Tap or press Space to reveal'}</p>
                            ) : (
                        ...
                        {flipped ? (
                            <div className="grid grid-cols-4 gap-2 mt-6">
                                {GRADES.map((g, i) => (
                                    ...
                                    >
                                        <span className="block">{tr ? ({ again: 'Tekrar', hard: 'Zor', good: 'İyi', easy: 'Kolay' }[g.key]) : g.label}</span>
                            ...
                            >
                                {tr ? 'Yanıtı Göster' : 'Reveal answer'}
                            </button>
                        )}
                    </>
                )}
