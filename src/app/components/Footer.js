...
export default function Footer() {
  ...
  const navLabel = (href, fallback) => ({ '/': 'home', '/about': 'about', '/portfolio': 'work', '/blog': 'writing', '/picks': 'picks', '/contact': 'contact' }[href] ? t({ '/': 'home', '/about': 'about', '/portfolio': 'work', '/blog': 'writing', '/picks': 'picks', '/contact': 'contact' }[href]) : fallback);
        ...
        <div className="page-content pb-4 pt-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-3">
              <span className="mini-app-icon-box flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border"><i className={`${localizedActiveMiniApp.icon} mini-app-accent`}></i></span>
              <div><p className="text-sm font-semibold text-white">{localizedActiveMiniApp.title}</p><p className="mini-app-muted text-sm">Bergaman Labs · {appVersion}</p></div>
            </div>
            <nav className="flex flex-wrap items-center gap-2" aria-label="Mini apps">
              {ACTIVE_MINI_APPS.map((sourceApp) => { const app = localizeMiniApp(sourceApp, locale); return <Link key={app.id} href={app.href} className={`mini-app-nav-item rounded-full px-3 py-1.5 text-sm ${pathname === app.href ? 'mini-app-nav-item-active' : ''}`}>{app.shortTitle}</Link>; })}
              <Link href="/portfolio" className="mini-app-nav-item rounded-full px-3 py-1.5 text-sm">{t('portfolio')}</Link>
            </nav>
          </div>
      ...
      <div className="page-content relative z-10 pt-7">
        <div className="mb-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <div className="text-center sm:text-left">
            <Link href="/" className="mb-2 inline-flex items-center gap-3"><i className="fas fa-dragon text-3xl text-[#e8c547]"></i><span><strong className="block text-2xl text-[#e8c547]">Bergaman</strong><small className="text-gray-400">{locale === 'tr' ? 'Ejderhanın Dünyası' : "The Dragon's Domain"}</small></span></Link>
          ...
          <FooterColumn icon="fas fa-link" title={labels.quick}>{NAV_LINKS.filter((link) => link.href !== '/').map((link) => <Link key={link.href} href={link.href} className="footer-link"><i className={`${link.icon} w-4 text-xs`}></i>{navLabel(link.href, link.label)}</Link>)}</FooterColumn>
        ...
        <div className="flex flex-col gap-3 border-t border-[#3e503e]/40 py-4 text-sm text-gray-400 sm:flex-row sm:items-center sm:justify-between">
          <div><p>© {currentYear} Ömer · {labels.copy} · v{appVersion}</p><p className="mt-1 text-xs text-gray-500">{locale === 'tr' ? 'Bergaman tarafından' : 'Made by Bergaman with'} <i className="fas fa-heart text-red-400"></i> <i className="fas fa-dragon text-[#e8c547]"></i></p></div>
          <div className="flex items-center gap-3"><a href="https://github.com/bergaman9/bergaman-dev/blob/main/LICENSE" target="_blank" rel="noopener noreferrer" className="rounded-full border border-[#3e503e]/50 px-3 py-2 text-xs hover:border-[#e8c547]/50 hover:text-[#e8c547]">{locale === 'tr' ? 'MIT Lisansı' : 'MIT License'}</a><button type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="min-h-11 rounded-lg px-3 text-sm hover:bg-[#e8c547]/10 hover:text-[#e8c547]">{labels.top} <i className="fas fa-arrow-up ml-1"></i></button></div>
        </div>
      </div>
    </footer>
  );
