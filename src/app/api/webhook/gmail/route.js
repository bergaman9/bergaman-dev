...
import { cleanEmailReply, extractContactId, extractEmailAddress, extractEmailDisplayName } from '@/lib/emailParsing';
...
export async function POST(request) {
  try {
    ...
    if (!contact && emailData.from) {
      ...
      if (senderEmail) contact = await Contact.findOne({ email: senderEmail });
    }
    ...
    if (!contact) {
      ...
      contact = new Contact({
        name: extractEmailDisplayName(emailData.from) || 'Unknown',
        email: senderEmail,
    ...
    // Clean email content
    const cleanContent = cleanEmailReply(emailData.text || emailData.html || '');
